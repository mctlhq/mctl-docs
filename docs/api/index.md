...
| Endpoint type | Limit |
|---------------|-------|
| Authenticated routes (including `POST /mcp`) | 300 requests/minute per user |
| Loopback MCP-internal REST (`127.0.0.1`, `::1`) | Excluded from the 300/min cap (does not consume the bucket) |
| Write (`POST /operations/*/execute` and related) | 20 requests/minute per user (loopback is **not** excluded) |
